  ...
  const handleCheckEmailVerification = async () => {
    ...
    try {
      ...
      if (res.isVerified && res.user) {
        ...
        setVerificationFeedback(res.message || 'Tu correo todavía no figura como verificado. Revisá tu casilla y hacé clic en el enlace de confirmación.');
      }
    } catch (err: any) {
      setVerificationFeedback(err.message || 'Error al comprobar el estado de verificación.');
    } finally {
      setSendingVerificationEmail(false);
    }
  };
            ...
            {currentUser.role !== 'admin' && !currentUser.emailVerified && (
              <div className="flex flex-wrap items-center gap-2">
                <button
                  ...
                  className="py-1.5 px-3 bg-rose-600 hover:bg-rose-500 rounded-xl text-[11px] font-bold text-white transition flex items-center gap-1.5 shrink-0 shadow"
                >
                  <CheckCircle2 className="w-3.5 h-3.5" />
                  <span>Comprobar Verificación</span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>{sendingVerificationEmail ? 'Enviando email...' : 'Reenviar enlace al correo'}</span>
                </button>
              </div>
            )}
          </div>
